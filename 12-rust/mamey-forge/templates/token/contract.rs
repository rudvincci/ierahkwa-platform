//! Fungible Token Contract for MameyFutureNode
//!
//! ERC-20-like fungible token with mint, transfer, burn, balance_of,
//! allowance, and governance (pause/unpause) support.
//!
...
pub fn init(name: String, symbol: String, decimals: u8, initial_supply: u64) -> Result<String, String> {
    ...
    validation::validate_decimals(decimals).map_err(|e| e.to_string())?;
...
#[wasm_bindgen]
pub fn name() -> String { storage::get_string(KEY_NAME).unwrap_or_default() }

#[wasm_bindgen]
pub fn symbol() -> String { storage::get_string(KEY_SYMBOL).unwrap_or_default() }

#[wasm_bindgen]
pub fn decimals() -> u8 { storage::get_u8(KEY_DECIMALS).unwrap_or(18) }

#[wasm_bindgen]
pub fn total_supply() -> u64 { storage::get_u64(KEY_TOTAL_SUPPLY).unwrap_or(0) }
...
/// Mint new tokens to the given address. Minter only.
#[wasm_bindgen]
...
pub fn transfer_from(from: String, to: String, amount: u64) -> Result<String, String> {
    ...
    let new_allowance = math::safe_sub(current_allowance, amount).map_err(|e| e.to_string())?;
    storage::set_u64(&allowance_key(&from, &spender), new_allowance).map_err(|e| e.to_string())?;

    host_functions::emit_event("Transfer", &serde_json::json!({
        "from": from, "to": to, "amount": amount, "spender": spender,
    }));
...
pub fn approve(spender: String, amount: u64) -> Result<String, String> {
    ...
    validation::validate_different_addresses(&owner, &spender).map_err(|e| e.to_string())?;
...
fn transfer_internal(from: &str, to: &str, amount: u64) -> Result<(), String> {
    ...
    if from_balance < amount { return Err("Insufficient balance".to_string()); }
...
fn require_not_paused() -> Result<(), String> {
    if storage::get_bool(KEY_PAUSED) { return Err("Contract is paused".to_string()); }
    Ok(())
}

fn require_owner() -> Result<(), String> {
    let caller = host_functions::get_caller();
    let owner = storage::get_string(KEY_OWNER).ok_or("No owner set".to_string())?;
    if caller != owner { return Err("Unauthorized: caller is not the owner".to_string()); }
    Ok(())
}

fn require_minter() -> Result<(), String> {
    let caller = host_functions::get_caller();
    let minter = storage::get_string(KEY_MINTER).ok_or("No minter set".to_string())?;
    if caller != minter { return Err("Unauthorized: caller is not the minter".to_string()); }
    Ok(())
}
