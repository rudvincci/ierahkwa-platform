...
/// <summary>
/// Base class for command integration tests with shared setup.
/// </summary>
public abstract class BaseCommandIntegrationTests : IAsyncLifetime
{
    ...
    protected BaseCommandIntegrationTests(PostgreSQLFixture fixture)
    {
        Fixture = fixture;
    }
    ...
    public virtual async Task InitializeAsync()
    {
        ...
        // Add event processor (in-memory implementation)
        services.AddSingleton<IEventProcessor>(provider =>
        {
            var logger = provider.GetRequiredService<ILogger<InMemoryEventProcessor>>();
            return new InMemoryEventProcessor(logger);
        });
...
/// <summary>
/// Simple in-memory event processor for testing.
/// </summary>
internal class InMemoryEventProcessor : IEventProcessor
{
    private readonly ILogger<InMemoryEventProcessor> _logger;
    private readonly List<IDomainEvent> _processedEvents = new();

    public InMemoryEventProcessor(ILogger<InMemoryEventProcessor> logger)
    {
        _logger = logger;
    }

    public Task ProcessAsync(IEnumerable<IDomainEvent> events, CancellationToken cancellationToken = default)
    {
        foreach (var @event in events)
        {
            _processedEvents.Add(@event);
            _logger.LogInformation("Processed event: {EventType}", @event.GetType().Name);
        }
        return Task.CompletedTask;
    }

    public IReadOnlyList<IDomainEvent> ProcessedEvents => _processedEvents.AsReadOnly();
}
