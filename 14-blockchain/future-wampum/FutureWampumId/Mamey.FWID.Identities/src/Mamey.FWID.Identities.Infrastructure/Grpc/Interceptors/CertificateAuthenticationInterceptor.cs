...
public class CertificateAuthenticationInterceptor : Interceptor
{
        ...
        try
        {
            ...
            if (_securityOptions.Certificate.Acl != null && _securityOptions.Certificate.Acl.Any())
            {
                ...
                if (acl.Permissions != null && acl.Permissions.Any())
                {
                    ...
                    if (httpContext != null)
                    {
                        // Set gRPC method in HTTP context for permission validator
                        var methodName = context.Method?.Split('/').LastOrDefault();
                        if (!string.IsNullOrEmpty(methodName))
                        {
                            httpContext.Items["grpc-method"] = methodName;
                        }
                        
                        // Set required permissions in HTTP context for permission validator
                        var requiredPermissions = GetRequiredPermissionsForMethod(methodName);
                        if (requiredPermissions != null)
                        {
                            httpContext.Items["RequiredPermissions"] = requiredPermissions;
                        }
                        
                        if (!_certificatePermissionValidator.HasAccess(certificate, acl.Permissions, httpContext))
                        {
    ...
    private HashSet<string>? GetRequiredPermissionsForMethod(string? methodName)
    {
        if (string.IsNullOrEmpty(methodName))
        {
            return null;
        ...
        var methodPermissions = new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase)
        {
        ...
        return methodPermissions.TryGetValue(methodName, out var permissions) ? permissions : null;
    }
}
