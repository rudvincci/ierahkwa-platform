...
public class AuthenticatorsServiceIntegrationTests : IClassFixture<AuthentikTestFixture>
{
    private readonly AuthentikTestFixture _fixture;
    private readonly IAuthentikClient? _client;

    public AuthenticatorsServiceIntegrationTests(AuthentikTestFixture fixture)
    {
        _fixture = fixture;
    ...
    public async Task HealthCheck_ServiceIsAccessible()
    {
        ...
        _client.Authenticators.Should().NotBeNull();
    }
    ...
    public async Task Service_WithInvalidToken_ThrowsAuthenticationException()
    {
        ...
        client.Authenticators.Should().NotBeNull();
    }

    // TODO: Add actual API method tests after code generation:
    // - GetDevicesAsync (TOTP, WebAuthn, etc.)
    // - CreateDeviceAsync
    // - UpdateDeviceAsync
    // - DeleteDeviceAsync
    // - etc.
