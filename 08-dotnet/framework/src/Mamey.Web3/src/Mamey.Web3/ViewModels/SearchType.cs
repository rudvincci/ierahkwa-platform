...
public class SendErc20TransactionViewModel : SendTransactionBaseViewModel
    {
        ...
        public async Task<string> SendTokenAsync()
        {
            var transferFunction =
                ...
                {
                    AmountToSend = new HexBigInteger(Nethereum.Web3.Web3.Convert.ToWei(AmountInEther)),
                    To = TransferTo,
                    FromAddress = Account,
                    Value = Nethereum.Web3.Web3.Convert.ToWei(TokenAmount, DecimalPlaces)
                };
            if (Gas != null)
            ...
            if (!string.IsNullOrEmpty(GasPrice))
            {
                ...
                transferFunction.GasPrice = new HexBigInteger(Nethereum.Web3.Web3.Convert.ToWei(GasPrice, UnitConversion.EthUnit.Gwei));
            }
