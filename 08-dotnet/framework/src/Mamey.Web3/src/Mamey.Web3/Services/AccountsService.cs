...
public class AccountsService : IAccountsService
{
    ...
    public async Task<decimal> GetAccountTokenBalanceAsync(string address, string contractAddress,
        int numberOfDecimals = 18)
    {
        var web3 = _web3ProviderService.GetWeb3();
        ...
        return Nethereum.Web3.Web3.Convert.FromWei(balance, numberOfDecimals);
    }
    ...
    public void UpdateTransactionInfo(TransactionInfo transactionInfo)
    {
        ...
        if (accountInfo.HasValue)
        {
            ...
            MessageBus.Current.SendMessage(new AccountTransactionCompleted(transactionInfo.TransactionHash,
                transactionInfo.AccountAddress, transactionInfo.TransactionReceipt.HasErrors()));
        }
    }
    ...
    public async Task<string> SendTransactionAsync(TransactionInput transactionInput)
    {
        ...
        {
            var hasPrivateKey =
                _privateKeyStorage.TryGetValue(transactionInput.From.ToLowerInvariant(), out var privateKey);
            if (hasPrivateKey)
            {
                var web3 = _web3ProviderService.GetWeb3(new Balances.Account(){ Address = accountInfo.Value.Address});
                var txnHash = await web3.Eth.TransactionManager.SendTransactionAsync(transactionInput)
    ...
    public async Task<string> SendTransactionAsync<TFunctionMessage>(string contractAddress,
        TFunctionMessage functionMessage) where TFunctionMessage : FunctionMessage, new()
    {
        var accountInfo = Accounts.Lookup(functionMessage.FromAddress.ToLowerInvariant());
        if (accountInfo.HasValue)
        {
            var hasPrivateKey =
                _privateKeyStorage.TryGetValue(functionMessage.FromAddress.ToLowerInvariant(), out var privateKey);
            if (hasPrivateKey)
            {
                var web3 = _web3ProviderService.GetWeb3(new Balances.Account());
        ...
        throw new ArgumentException($@"Account address: {functionMessage.FromAddress}, not found",
            nameof(functionMessage));
    }
}
...
{
    [Parameter("address", "_owner", 1)] public virtual string Owner { get; set; }
}
...
{
    [Parameter("address", "_to", 1)] public virtual string To { get; set; }
    [Parameter("uint256", "_value", 2)] public virtual BigInteger Value { get; set; }
}