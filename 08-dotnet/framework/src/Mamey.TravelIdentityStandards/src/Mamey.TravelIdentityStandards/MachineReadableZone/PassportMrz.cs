...
/// 
/// </summary>
public class PassportMrz(string line1, string line2)
    : Mrz(DocumentType.Passport, Lines, LineLenghtCount)
{
    private const int Lines = 2;
    private const int LineLenghtCount = 44;
    private const string Line1Regex = @"^P<[A-Z]{3}[A-Z<]+<<[A-Z<]+$";
    private const string Line2Regex = @"^[0-9]{6}[0-9][MF<][0-9]{6}[0-9][A-Z]{3}[A-Z0-9<]{7}[0-9]$";
