...
using System.Text;
using Mamey.Auth.Jwt.Handlers;
using Mamey.Auth.Jwt.Services;
using Mamey.Modules;
...
[assembly: InternalsVisibleTo("Mamey.Azure.Identity")]
...
public static class Extensions
{
        ...
        Action<JwtBearerOptions> optionsFactory = null, IList<IModule> modules = null)
    {
        ...
        if (options.Certificate is not null)
        {
            ...
            {
                // Try to load certificate - handle cases where file doesn't exist (e.g., in tests)
                try
                {
                    // Check if certificate file exists before trying to load it
                    // This is especially important for tests where certificates might not be available
                    if (File.Exists(options.Certificate.Location))
                    {
                    ...
                    {
                        Console.WriteLine($"Warning: X.509 certificate file not found at location: '{options.Certificate.Location}'. Skipping certificate authentication.");
                    }
                }
                catch (DirectoryNotFoundException ex)
                {
                    Console.WriteLine($"Warning: X.509 certificate directory not found: '{options.Certificate.Location}'. Skipping certificate authentication. Error: {ex.Message}");
                    // Continue without certificate - will fall back to issuerSigningKey if available
                }
                catch (FileNotFoundException ex)
                {
                    Console.WriteLine($"Warning: X.509 certificate file not found: '{options.Certificate.Location}'. Skipping certificate authentication. Error: {ex.Message}");
                    // Continue without certificate - will fall back to issuerSigningKey if available
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Warning: Failed to load X.509 certificate from location: '{options.Certificate.Location}'. Skipping certificate authentication. Error: {ex.Message}");
                    // Continue without certificate - will fall back to issuerSigningKey if available
                }
            }
