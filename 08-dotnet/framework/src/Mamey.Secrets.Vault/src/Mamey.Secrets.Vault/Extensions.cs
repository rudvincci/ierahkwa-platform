using Mamey.Secrets.Vault.Internals;
...
public static class Extensions
{
    ...
    public static WebApplicationBuilder UseVault(this WebApplicationBuilder builder, string keyValuePath = null, string sectionName = SectionName)
    {
        // Add Vault services to the DI container
        builder.Services.AddVault(sectionName);

        // Configure Vault options and add to the configuration
        var configuration = builder.Configuration;
        var options = configuration.GetOptions<VaultOptions>(sectionName);

        if (options.Enabled)
        {
            // Load Vault configuration asynchronously and block until completion
            configuration.AddVaultAsync(options, keyValuePath).GetAwaiter().GetResult();
        }
