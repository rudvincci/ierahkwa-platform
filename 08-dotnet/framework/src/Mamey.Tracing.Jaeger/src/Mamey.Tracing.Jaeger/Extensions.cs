using Mamey.Tracing.Jaeger.Builders;
using Mamey.Tracing.Jaeger.Tracers;
using Jaeger;
...
using OpenTracing;
using Mamey.OpenTracingContrib.Configuration;
using OpenTracing.Util;

namespace Mamey.Tracing.Jaeger;
...
public static class Extensions
{
    ...
    public static IMameyBuilder AddJaeger(this IMameyBuilder builder, string sectionName = SectionName,
    ...
    public static IMameyBuilder AddJaeger(this IMameyBuilder builder,
    ...
    public static IMameyBuilder AddJaeger(this IMameyBuilder builder, JaegerOptions options,
        string sectionName = SectionName, Action<IOpenTracingBuilder> openTracingBuilder = null)
    {
        ...
        if (!options.Enabled)
        {
            var defaultTracer = MameyDefaultTracer.Create();
        ...
        builder.Services.AddOpenTracing(x =>
        {
            openTracingBuilder?.Invoke(x);
        });

        builder.Services.AddSingleton<ITracer>(sp =>
        {
            var loggerFactory = sp.GetRequiredService<ILoggerFactory>();
            var logger = loggerFactory.CreateLogger("Mamey.Tracing.Jaeger");
            
            logger.LogInformation("Initializing Jaeger tracer for service: {ServiceName}", options.ServiceName);
            ...
            logger.LogInformation("Jaeger sender type: {SenderType}", senderType);
            
            // Ensure UDP defaults are set when using UDP sender
            if (senderType == "udp")
            {
                if (string.IsNullOrWhiteSpace(options.UdpHost))
                {
                    options.UdpHost = "localhost";
                    logger.LogWarning("UDP host not configured, using default: localhost");
                }
                
                if (options.UdpPort <= 0)
                {
                    options.UdpPort = 6831;
                    logger.LogWarning("UDP port not configured, using default: 6831");
                }
                
                logger.LogInformation("UDP configuration - Host: {Host}, Port: {Port}, MaxPacketSize: {MaxPacketSize}", 
                    options.UdpHost, options.UdpPort, maxPacketSize);
            }
            ...
            logger.LogInformation("Jaeger sampler: {Sampler}", options.Sampler ?? "const");
            ...
            logger.LogInformation("Jaeger tracer initialized and registered successfully");
    ...
    public static IApplicationBuilder UseJaeger(this IApplicationBuilder app)
    {
        ...
        // Check if JaegerOptions is registered before trying to use it
        // This is especially important for tests where Jaeger might not be configured
        using var scope = app.ApplicationServices.CreateScope();
        var options = scope.ServiceProvider.GetService<JaegerOptions>();
        
        if (options == null)
        {
            // JaegerOptions not registered - skip Jaeger initialization
            // This is expected in test environments where Jaeger might not be configured
            return app;
        }
