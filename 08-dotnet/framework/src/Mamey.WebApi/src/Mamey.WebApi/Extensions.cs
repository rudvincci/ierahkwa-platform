...
using Mamey.WebApi.Formatters;
using Mamey.WebApi.Requests;
...
public static class Extensions
{
    ...
    public static IMameyBuilder AddWebApi(this IMameyBuilder builder, Action<IMvcCoreBuilder> configureMvc = null,
        IJsonSerializer jsonSerializer = null, string sectionName = SectionName)
    {
        ...
        if (jsonSerializer.GetType().Namespace?.Contains("Newtonsoft") == true)
        {
            ...
            // Try to configure IIS ServerOptions if available, gracefully skip if not
            //ConfigureIISServerOptionsIfAvailable(builder.Services);
        }
    ...
    public static IMameyBuilder AddErrorHandler<T>(this IMameyBuilder builder)
    ...
    // private static void ConfigureIISServerOptionsIfAvailable(IServiceCollection services)
    // {
    //     try
    //     {
    //         // Try to load IIS ServerOptions type dynamically
    //         var iisServerOptionsType = Type.GetType("Microsoft.AspNetCore.Server.IIS.IISServerOptions, Microsoft.AspNetCore.Server.IIS", false);
    //         if (iisServerOptionsType == null)
    //         {
    //             return;
    //         }
    //
    //         // Get the Configure method from OptionsServiceCollectionExtensions
    ...
    //         {
    //             // Create generic method for IISServerOptions type
    //             var genericMethod = configureMethod.MakeGenericMethod(iisServerOptionsType);
    //             
    //             // Create an action that sets AllowSynchronousIO to true
    //             var actionType = typeof(Action<>).MakeGenericType(iisServerOptionsType);
    //             var property = iisServerOptionsType.GetProperty("AllowSynchronousIO");
    //             
    //             if (property != null)
    //             {
    //                 var action = Delegate.CreateDelegate(actionType, 
    //                     new Action<object>(options => property.SetValue(options, true)));
    //                 
    //                 genericMethod.Invoke(null, new object[] { services, action });
    //             }
    //         }
    //     }
    //     catch
    //     {
    //         // IIS package not available, skip IIS configuration
    //     }
    // }
