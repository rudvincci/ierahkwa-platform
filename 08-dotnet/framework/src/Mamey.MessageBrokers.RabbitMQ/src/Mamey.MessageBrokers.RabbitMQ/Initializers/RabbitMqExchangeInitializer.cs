...
namespace Mamey.MessageBrokers.RabbitMQ.Initializers;
...
public class RabbitMqExchangeInitializer : IInitializer
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_options.Exchange?.Declare == true)
        {
            ...
            await channel.ExchangeDeclareAsync(_options.Exchange.Name, _options.Exchange.Type, _options.Exchange.Durable,
                _options.Exchange.AutoDelete);

            if (_options.DeadLetter?.Enabled is true && _options.DeadLetter?.Declare is true)
            {
                await channel.ExchangeDeclareAsync($"{_options.DeadLetter.Prefix}{_options.Exchange.Name}{_options.DeadLetter.Suffix}",
