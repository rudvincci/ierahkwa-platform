...
namespace Mamey.MessageBrokers.RabbitMQ.Internals;
...
internal sealed class RabbitMqBackgroundService : BackgroundService
{
    ...
    private async Task SubscribeAsync(IMessageSubscriber messageSubscriber)
    {
        ...
        if (_options.DeadLetter?.Enabled is true)
        {
            if (_options.DeadLetter.Declare)
            {
                ...
                await channel.QueueDeclareAsync(deadLetterQueue, _options.DeadLetter.Durable, _options.DeadLetter.Exclusive,
    ...
    private async Task TryHandleAsync(IChannel channel, object message, string messageId, string correlationId,
    ...
    public override void Dispose()
    {
        ...
        try
        {
            _consumerConnection.CloseAsync().GetAwaiter().GetResult();
