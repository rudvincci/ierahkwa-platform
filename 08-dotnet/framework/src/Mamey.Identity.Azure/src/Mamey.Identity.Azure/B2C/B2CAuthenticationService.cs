...
namespace Mamey.Identity.Azure.B2C;
...
public class B2CAuthenticationService : IAzureAuthService
{
    ...
    public async Task<AuthenticatedUser?> GetAuthenticatedUserAsync(ClaimsPrincipal claimsPrincipal)
    {
        try
        {
            ...
            {
                _logger.LogWarning("No user ID found in claims principal for B2C user");
                return null;
            }
        ...
        {
            _logger.LogError(ex, "Error getting authenticated B2C user from Azure AD");
            return null;
        }
    }
    ...
    public async Task<AzureUserInfo?> GetUserInfoAsync(string userId)
    {
        ...
        {
            _logger.LogError(ex, "Error getting B2C user info from Microsoft Graph for user {UserId}", userId);
            return null;
        }
    }
    ...
    public async Task<IEnumerable<AzureGroupInfo>> GetUserGroupsAsync(string userId)
    {
        ...
        {
            _logger.LogError(ex, "Error getting B2C user groups from Microsoft Graph for user {UserId}", userId);
            return Enumerable.Empty<AzureGroupInfo>();
        }
    }
    ...
    public async Task<bool> ValidateTokenAsync(string token)
    {
        try
        {
            // B2C specific token validation logic
            _logger.LogDebug("B2C token validation requested");
            return await Task.FromResult(!string.IsNullOrEmpty(token));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error validating B2C token");
            return false;
        }
    }
    ...
    public async Task<string> GetSignOutUrlAsync(string userId)
    {
        try
        {
            ...
            _logger.LogDebug("Generated B2C sign out URL for user {UserId}", userId);
            return await Task.FromResult(signOutUrl);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error generating B2C sign out URL for user {UserId}", userId);
            return string.Empty;
        }
    }
}
