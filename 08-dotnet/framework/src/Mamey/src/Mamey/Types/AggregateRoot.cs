...
public abstract class AggregateRoot<T> : IIdentifiable<T>
{
    ...
    public (bool, List<ValidationResult>?) Validate(bool throwException = true)
    {
        ...
        if (TryValidate(this, out results, throwException))
        {
            return (true, null);
        };
    ...
    protected bool TryValidateProperty<TEntity>(object propertyValue, Expression<Func<TEntity, object>> predicate, 
        out List<ValidationResult> results, bool throwException = true)
    {
        results = new List<ValidationResult>();
