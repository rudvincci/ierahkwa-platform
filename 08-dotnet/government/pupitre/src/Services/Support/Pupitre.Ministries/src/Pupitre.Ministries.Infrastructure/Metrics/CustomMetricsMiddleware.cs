...
namespace Pupitre.Ministries.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/ministries")] = Query(typeof(GetMinistryData).Name),
            [GetKey("POST", "/ministries")] = Command(typeof(AddMinistryData).Name),
            [GetKey("PUT", "/ministries")] = Command(typeof(UpdateMinistryData).Name),
            [GetKey("DELETE", "/ministries")] = Command(typeof(DeleteMinistryData).Name)
        };
