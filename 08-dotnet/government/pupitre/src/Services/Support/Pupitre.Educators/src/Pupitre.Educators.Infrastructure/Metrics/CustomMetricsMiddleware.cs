...
namespace Pupitre.Educators.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/educators")] = Query(typeof(GetEducator).Name),
            [GetKey("POST", "/educators")] = Command(typeof(AddEducator).Name),
            [GetKey("PUT", "/educators")] = Command(typeof(UpdateEducator).Name),
            [GetKey("DELETE", "/educators")] = Command(typeof(DeleteEducator).Name)
        };
