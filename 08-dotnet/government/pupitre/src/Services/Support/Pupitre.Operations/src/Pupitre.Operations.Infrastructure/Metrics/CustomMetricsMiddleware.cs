...
namespace Pupitre.Operations.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/operations")] = Query(typeof(GetOperationMetric).Name),
            [GetKey("POST", "/operations")] = Command(typeof(AddOperationMetric).Name),
            [GetKey("PUT", "/operations")] = Command(typeof(UpdateOperationMetric).Name),
            [GetKey("DELETE", "/operations")] = Command(typeof(DeleteOperationMetric).Name)
        };
