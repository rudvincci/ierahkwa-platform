...
namespace Pupitre.Bookstore.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/bookstore")] = Query(typeof(GetBook).Name),
            [GetKey("POST", "/bookstore")] = Command(typeof(AddBook).Name),
            [GetKey("PUT", "/bookstore")] = Command(typeof(UpdateBook).Name),
            [GetKey("DELETE", "/bookstore")] = Command(typeof(DeleteBook).Name)
        };
