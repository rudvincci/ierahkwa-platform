...
namespace Pupitre.Compliance.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/compliance")] = Query(typeof(GetComplianceRecord).Name),
            [GetKey("POST", "/compliance")] = Command(typeof(AddComplianceRecord).Name),
            [GetKey("PUT", "/compliance")] = Command(typeof(UpdateComplianceRecord).Name),
            [GetKey("DELETE", "/compliance")] = Command(typeof(DeleteComplianceRecord).Name)
        };
