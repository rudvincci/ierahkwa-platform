...
namespace Pupitre.Aftercare.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/aftercare")] = Query(typeof(GetAftercarePlan).Name),
            [GetKey("POST", "/aftercare")] = Command(typeof(AddAftercarePlan).Name),
            [GetKey("PUT", "/aftercare")] = Command(typeof(UpdateAftercarePlan).Name),
            [GetKey("DELETE", "/aftercare")] = Command(typeof(DeleteAftercarePlan).Name)
        };
