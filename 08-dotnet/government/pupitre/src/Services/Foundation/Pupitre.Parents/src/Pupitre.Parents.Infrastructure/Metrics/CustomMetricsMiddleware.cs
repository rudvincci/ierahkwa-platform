...
namespace Pupitre.Parents.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/parents")] = Query(typeof(GetParent).Name),
            [GetKey("POST", "/parents")] = Command(typeof(AddParent).Name),
            [GetKey("PUT", "/parents")] = Command(typeof(UpdateParent).Name),
            [GetKey("DELETE", "/parents")] = Command(typeof(DeleteParent).Name)
        };
