...
namespace Pupitre.Curricula.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/curricula")] = Query(typeof(GetCurriculum).Name),
            [GetKey("POST", "/curricula")] = Command(typeof(AddCurriculum).Name),
            [GetKey("PUT", "/curricula")] = Command(typeof(UpdateCurriculum).Name),
            [GetKey("DELETE", "/curricula")] = Command(typeof(DeleteCurriculum).Name)
        };
