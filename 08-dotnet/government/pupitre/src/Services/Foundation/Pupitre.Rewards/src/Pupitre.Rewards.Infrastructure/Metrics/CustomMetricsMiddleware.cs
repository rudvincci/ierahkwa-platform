...
namespace Pupitre.Rewards.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/rewards")] = Query(typeof(GetReward).Name),
            [GetKey("POST", "/rewards")] = Command(typeof(AddReward).Name),
            [GetKey("PUT", "/rewards")] = Command(typeof(UpdateReward).Name),
            [GetKey("DELETE", "/rewards")] = Command(typeof(DeleteReward).Name)
        };
