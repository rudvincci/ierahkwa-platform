...
namespace Pupitre.GLEs.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/gles")] = Query(typeof(GetGLE).Name),
            [GetKey("POST", "/gles")] = Command(typeof(AddGLE).Name),
            [GetKey("PUT", "/gles")] = Command(typeof(UpdateGLE).Name),
            [GetKey("DELETE", "/gles")] = Command(typeof(DeleteGLE).Name)
        };
