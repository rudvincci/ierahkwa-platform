using System.Net;
using System.Text;
using Pupitre.Lessons.Api;
using Pupitre.Lessons.Application.Commands;
using Pupitre.Lessons.Contracts.Commands;
using Pupitre.Lessons.Infrastructure.Mongo.Documents;
using Pupitre.Lessons.Tests.Shared.Factories;
using Pupitre.Lessons.Tests.Shared.Fixtures;
using Newtonsoft.Json;
using Shouldly;
using Xunit;

namespace Pupitre.Lessons.Tests.EndToEnd.Sync
{
    public class AddLessonTests : IDisposable, IClassFixture<MameyApplicationFactory<Program>>
    {
        private Task<HttpResponseMessage> Act(AddLesson command) 
            => _httpClient.PostAsync("lessons", GetContent(command));

        [Fact]
        public async Task add_lesson_endpoint_should_return_http_status_code_created()
        {
            var command = new AddLesson(Guid.NewGuid(), "name", new []{"tag"});

            var response = await Act(command);
            
            response.ShouldNotBeNull();
            response.StatusCode.ShouldBe(HttpStatusCode.Created);
        }
        
        [Fact]
        public async Task add_lesson_endpoint_should_return_location_header_with_correct_lesson_id()
        {
            var command = new AddLesson(Guid.NewGuid(), "name", new []{"tag"});

            var response = await Act(command);

            var locationHeader = response.Headers.FirstOrDefault(h => h.Key == "Location").Value.First();
            
            locationHeader.ShouldNotBeNull();
            locationHeader.ShouldBe($"lessons/{command.Id}");
        }

        [Fact]
        public async Task add_lesson_endpoint_should_add_document_with_given_id_to_database()
        {
            var command = new AddLesson(Guid.NewGuid(), "name", new []{"tag"});

            await Act(command);

            var document = await _mongoDbFixture.GetAsync(command.Id);
            
            document.ShouldNotBeNull();
            document.Id.ShouldBe(command.Id);
            document.Tags.ShouldBe(command.Tags);
        }

        #region Arrange

        private readonly HttpClient _httpClient;
        private readonly MongoDbFixture<LessonDocument, Guid> _mongoDbFixture;
        
        public AddLessonTests(MameyApplicationFactory<Program> factory)
        {
            _mongoDbFixture = new MongoDbFixture<LessonDocument, Guid>("lessons");
            factory.Server.AllowSynchronousIO = true;
            _httpClient = factory.CreateClient();
        }
        
        public void Dispose()
        {
            _mongoDbFixture.Dispose();
        }
        
        private static StringContent GetContent(object value) 
            => new StringContent(JsonConvert.SerializeObject(value), Encoding.UTF8, "application/json");
        
        #endregion
    }
}