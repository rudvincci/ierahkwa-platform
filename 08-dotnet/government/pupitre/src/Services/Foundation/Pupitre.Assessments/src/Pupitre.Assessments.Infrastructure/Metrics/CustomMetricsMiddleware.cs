...
namespace Pupitre.Assessments.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/assessments")] = Query(typeof(GetAssessment).Name),
            [GetKey("POST", "/assessments")] = Command(typeof(AddAssessment).Name),
            [GetKey("PUT", "/assessments")] = Command(typeof(UpdateAssessment).Name),
            [GetKey("DELETE", "/assessments")] = Command(typeof(DeleteAssessment).Name)
        };
