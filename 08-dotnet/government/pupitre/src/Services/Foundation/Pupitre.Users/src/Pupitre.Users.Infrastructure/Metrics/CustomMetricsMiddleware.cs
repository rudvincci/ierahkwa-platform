...
namespace Pupitre.Users.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/users")] = Query(typeof(GetUser).Name),
            [GetKey("POST", "/users")] = Command(typeof(AddUser).Name),
            [GetKey("PUT", "/users")] = Command(typeof(UpdateUser).Name),
            [GetKey("DELETE", "/users")] = Command(typeof(DeleteUser).Name)
        };
