...
namespace Pupitre.AIAssessments.Infrastructure.PostgreSQL;

internal class AIAssessmentsDbContext : DbContext
{
    public DbSet<InboxMessage> Inbox { get; set; }
    public DbSet<OutboxMessage> Outbox { get; set; }
    public DbSet<AIAssessment> AIAssessments { get; set; }

    public AIAssessmentsDbContext(DbContextOptions<AIAssessmentsDbContext> options) : base(options)
    {
    }

    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.HasDefaultSchema("aiassessments");
        ...
        modelBuilder.Entity<OutboxMessage>()
            ...
            .HasConversion<JsonValueConverter<object>>();
        modelBuilder.ApplyConfigurationsFromAssembly(typeof(AIAssessmentsDbContext).Assembly);
