using System.Net;
using System.Text;
using Pupitre.AIRecommendations.Api;
using Pupitre.AIRecommendations.Application.Commands;
using Pupitre.AIRecommendations.Contracts.Commands;
using Pupitre.AIRecommendations.Infrastructure.Mongo.Documents;
using Pupitre.AIRecommendations.Tests.Shared.Factories;
using Pupitre.AIRecommendations.Tests.Shared.Fixtures;
using Newtonsoft.Json;
using Shouldly;
using Xunit;

namespace Pupitre.AIRecommendations.Tests.EndToEnd.Sync
{
    public class AddAIRecommendationTests : IDisposable, IClassFixture<MameyApplicationFactory<Program>>
    {
        private Task<HttpResponseMessage> Act(AddAIRecommendation command) 
            => _httpClient.PostAsync("airecommendations", GetContent(command));

        [Fact]
        public async Task add_airecommendation_endpoint_should_return_http_status_code_created()
        {
            var command = new AddAIRecommendation(Guid.NewGuid(), "name", new []{"tag"});

            var response = await Act(command);
            
            response.ShouldNotBeNull();
            response.StatusCode.ShouldBe(HttpStatusCode.Created);
        }
        
        [Fact]
        public async Task add_airecommendation_endpoint_should_return_location_header_with_correct_airecommendation_id()
        {
            var command = new AddAIRecommendation(Guid.NewGuid(), "name", new []{"tag"});

            var response = await Act(command);

            var locationHeader = response.Headers.FirstOrDefault(h => h.Key == "Location").Value.First();
            
            locationHeader.ShouldNotBeNull();
            locationHeader.ShouldBe($"airecommendations/{command.Id}");
        }

        [Fact]
        public async Task add_airecommendation_endpoint_should_add_document_with_given_id_to_database()
        {
            var command = new AddAIRecommendation(Guid.NewGuid(), "name", new []{"tag"});

            await Act(command);

            var document = await _mongoDbFixture.GetAsync(command.Id);
            
            document.ShouldNotBeNull();
            document.Id.ShouldBe(command.Id);
            document.Tags.ShouldBe(command.Tags);
        }

        #region Arrange

        private readonly HttpClient _httpClient;
        private readonly MongoDbFixture<AIRecommendationDocument, Guid> _mongoDbFixture;
        
        public AddAIRecommendationTests(MameyApplicationFactory<Program> factory)
        {
            _mongoDbFixture = new MongoDbFixture<AIRecommendationDocument, Guid>("airecommendations");
            factory.Server.AllowSynchronousIO = true;
            _httpClient = factory.CreateClient();
        }
        
        public void Dispose()
        {
            _mongoDbFixture.Dispose();
        }
        
        private static StringContent GetContent(object value) 
            => new StringContent(JsonConvert.SerializeObject(value), Encoding.UTF8, "application/json");
        
        #endregion
    }
}