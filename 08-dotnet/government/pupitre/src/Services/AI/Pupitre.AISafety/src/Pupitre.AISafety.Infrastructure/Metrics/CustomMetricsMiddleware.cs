...
namespace Pupitre.AISafety.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/aisafety")] = Query(typeof(GetSafetyCheck).Name),
            [GetKey("POST", "/aisafety")] = Command(typeof(AddSafetyCheck).Name),
            [GetKey("PUT", "/aisafety")] = Command(typeof(UpdateSafetyCheck).Name),
            [GetKey("DELETE", "/aisafety")] = Command(typeof(DeleteSafetyCheck).Name)
        };
