...
internal sealed class ExceptionToResponseMapper : IExceptionToResponseMapper
{
    ...
    public ExceptionResponse Map(Exception exception)
        => exception switch
        {
            ...
            MameyException ex => new ExceptionResponse(new { code = GetCode(ex), reason = ex.Message },
                HttpStatusCode.BadRequest),
    ...
    private static string GetCode(Exception exception)
    {
        ...
        var exceptionCode = exception switch
        {
            DomainException domainException when !string.IsNullOrWhiteSpace(domainException.Code) => domainException
                .Code,
            MameyException appException when !string.IsNullOrWhiteSpace(appException.Code) => appException.Code,
