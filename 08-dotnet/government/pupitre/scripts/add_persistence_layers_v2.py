...
Version 2: Uses strongly-typed IDs and proper interface signatures.
"""
...
{{
    ...
    public async Task<{entity_name}> GetAsync({entity_name}Id id, CancellationToken cancellationToken = default)
    {{
    ...
    public async Task DeleteAsync({entity_name}Id id, CancellationToken cancellationToken = default)
    {{
        var key = GetKey(id.Value);
        await _database.KeyDeleteAsync(key);
    }}

    public async Task<bool> ExistsAsync({entity_name}Id id, CancellationToken cancellationToken = default)
    {{
        var key = GetKey(id.Value);
