...
using Mamey.Government.Identity.Application.Queries;
...
namespace Mamey.Government.Identity.Infrastructure.Metrics
{
    public class CustomMetricsMiddleware : IMiddleware
    {
        ...
        {
            [GetKey("GET", "/identity")] = Query(typeof(GetSubject).Name),
            [GetKey("POST", "/identity")] = Command(typeof(AddSubject).Name),
            [GetKey("PUT", "/identity")] = Command(typeof(UpdateSubject).Name),
            [GetKey("DELETE", "/identity")] = Command(typeof(DeleteSubject).Name)
        };
