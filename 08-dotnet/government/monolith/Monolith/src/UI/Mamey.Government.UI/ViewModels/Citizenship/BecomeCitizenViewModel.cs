...
namespace Mamey.Government.UI.ViewModels.Citizenship;
...
public sealed class BecomeCitizenViewModel : ReactiveObject
{
    ...
    /// <summary>
    /// Loads application data from ApplicationModel into the view model.
    /// </summary>
    public void LoadFromApplication(ApplicationModel application)
    {
        // Basic information
        ...
        PhoneNumber = application.Phone ?? string.Empty;

        // Address (from legacy or AddressDto)
        if (application.Address != null)
        {
            AddressLine1 = application.Address.Line ?? string.Empty;
            City = application.Address.City ?? string.Empty;
            Region = application.Address.State ?? string.Empty;
            PostalCode = application.Address.PostalCode ?? application.Address.Zip5 ?? string.Empty;
        }
        else
        {
            AddressLine1 = application.Street ?? string.Empty;
            City = application.City ?? string.Empty;
            Region = application.State ?? string.Empty;
        ...
        {
            PlaceOfBirth = application.PersonalDetails.PlaceOfBirth ?? string.Empty;
            EyeColor = application.PersonalDetails.EyeColor ?? string.Empty;
            HairColor = application.PersonalDetails.HairColor ?? string.Empty;
            Height = application.PersonalDetails.HeightInInches.ToString() ?? string.Empty;
            Sex = application.PersonalDetails.Gender ?? string.Empty;
            MaritalStatus = application.PersonalDetails.MaritalStatus ?? string.Empty;
            SpouseName = application.PersonalDetails.SpouseName is null ? string.Empty : $"{application.PersonalDetails.SpouseName.LastName}, {application.PersonalDetails.SpouseName.FirstName} {application.PersonalDetails.SpouseName.MiddleName}";
            PreviousNames = string.Join(", ", application.PersonalDetails.Aliases ?? Array.Empty<string>());
        }

        // Contact information
        if (application.ContactInformation != null)
        {
            EmergencyContactName = application.ContactInformation.EmergencyContactName ?? string.Empty;
            EmergencyContactPhone = application.ContactInformation.EmergencyContactPhoneNumber ?? string.Empty;
            EmergencyContactEmail = application.ContactInformation.EmergencyContactEmail ?? string.Empty;
            EmergencyContactAddress = application.ContactInformation.EmergencyContactAddress;
            EmergencyContactRelationship = application.ContactInformation.EmergencyContactRelationship ?? string.Empty;
        }

        // Foreign identification
        if (application.ForeignIdentification != null)
        {
            PassportNumber = application.ForeignIdentification.Number ?? string.Empty;
            PassportIssuingCountry = application.ForeignIdentification.IssuingCountry ?? string.Empty;
            PassportExpiryDate = application.ForeignIdentification.ExpiryDate;
        }

        // Flags
        HasCriminalRecord = application.HaveCriminalRecord ?? false;
        if (application.CriminalHistory != null && application.CriminalHistory.Any())
        {
            CriminalRecordDetails = string.Join("; ", application.CriminalHistory.Select(c => $"{c.Charge} - {(c.Convicted ? "Convicted": "Not Guilty")}"));
        }

        // References (map first two if available)
        if (application.References != null && application.References.Count > 0)
        {
            var ref1 = application.References[0];
            Reference1Name = ref1.Name?.FirstName + " " + ref1.Name?.LastName ?? string.Empty;
            Reference1Relationship = ref1.Relationship ?? string.Empty;
            Reference1Phone = ref1.Phone ?? string.Empty;
            Reference1Address = FormatAddress(ref1.Address);

            if (application.References.Count > 1)
            {
                var ref2 = application.References[1];
                Reference2Name = ref2.Name?.FirstName + " " + ref2.Name?.LastName ?? string.Empty;
                Reference2Relationship = ref2.Relationship ?? string.Empty;
                Reference2Phone = ref2.Phone ?? string.Empty;
                Reference2Address = FormatAddress(ref2.Address);
            }
        }

        // Note: Many fields in the view model don't have direct mappings from ApplicationModel
        // These would need to be filled in by the user or from other sources
    }
    ...
    private string FormatAddress(AddressDto? address)
    {
        ...
        if (!string.IsNullOrEmpty(address.PostalCode ?? address.Zip5)) parts.Add(address.PostalCode ?? address.Zip5);
