...
internal class CompositePassportRepository : IPassportRepository
{
    private readonly PassportMongoRepository _mongoRepo;
    private readonly PassportRedisRepository _redisRepo;
    private readonly PassportPostgresRepository _postgresRepo;
    private readonly ILogger<CompositePassportRepository> _logger;

    public CompositePassportRepository(
        PassportMongoRepository mongoRepo,
        PassportRedisRepository redisRepo,
        PassportPostgresRepository postgresRepo,
        ILogger<CompositePassportRepository> logger)
    {
    ...
    public async Task AddAsync(Passport entity, CancellationToken cancellationToken = default)
    {
    ...
    public async Task UpdateAsync(Passport entity, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<Passport?> GetByPassportNumberAsync(PassportNumber passportNumber, CancellationToken cancellationToken = default)
    {
        try
        {
            var fromMongo = await _mongoRepo.GetByPassportNumberAsync(passportNumber, cancellationToken);
            if (fromMongo != null) return fromMongo;
        }
        catch (Exception ex) { _logger.LogWarning(ex, "Mongo lookup failed, trying Postgres"); }
        
        return await _postgresRepo.GetByPassportNumberAsync(passportNumber, cancellationToken);
    }
