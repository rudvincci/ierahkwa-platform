using Mamey.Government.Modules.Passports.Core.Domain.Entities;
using Mamey.Government.Modules.Passports.Core.Domain.ValueObjects;

namespace Mamey.Government.Modules.Passports.Core.Sync;

internal interface IReadModelSyncService
{
    Task SyncPassportAsync(Passport passport, CancellationToken cancellationToken = default);
    Task RemovePassportAsync(PassportId passportId, CancellationToken cancellationToken = default);
    Task<SyncStatus> GetSyncStatusAsync(CancellationToken cancellationToken = default);
}

public class SyncStatus
{
    public DateTime LastSyncTime { get; set; }
    public long TotalSynced { get; set; }
    public long TotalFailed { get; set; }
    public TimeSpan AverageSyncLatency { get; set; }
    public bool IsHealthy { get; set; }
}
