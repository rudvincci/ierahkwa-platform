...
namespace Mamey.Government.Modules.Passports.Core.Sync;

internal class ReadModelSyncService : IReadModelSyncService
{
    private readonly IPassportRepository _postgresRepository;
    private readonly PassportMongoRepository _mongoRepository;
    ...
    public ReadModelSyncService(
        IPassportRepository postgresRepository,
        PassportMongoRepository mongoRepository,
    ...
    public async Task SyncPassportAsync(Passport passport, CancellationToken cancellationToken = default)
    {
        var stopwatch = Stopwatch.StartNew();
        
        try
        {
            var exists = await _mongoRepository.ExistsAsync(passport.Id, cancellationToken);
            
            if (exists)
            {
                await _mongoRepository.UpdateAsync(passport, cancellationToken);
            }
            else
            {
                await _mongoRepository.AddAsync(passport, cancellationToken);
            }
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Failed to sync Passport {PassportId} to MongoDB", passport.Id.Value);
            throw;
        }
    }

    public async Task RemovePassportAsync(PassportId passportId, CancellationToken cancellationToken = default)
    {
        var stopwatch = Stopwatch.StartNew();
        
        try
        {
            await _mongoRepository.DeleteAsync(passportId, cancellationToken);
            stopwatch.Stop();
            UpdateMetrics(stopwatch.Elapsed, success: true);
        }
        catch (Exception ex)
        {
            stopwatch.Stop();
            UpdateMetrics(stopwatch.Elapsed, success: false);
            _logger.LogError(ex, "Failed to remove Passport {PassportId} from MongoDB", passportId.Value);
            throw;
        }
    }
