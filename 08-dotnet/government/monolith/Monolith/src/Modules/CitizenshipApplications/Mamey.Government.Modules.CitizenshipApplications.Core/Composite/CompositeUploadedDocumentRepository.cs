...
internal class CompositeUploadedDocumentRepository : IUploadedDocumentRepository
{
    private readonly UploadedDocumentMongoRepository _mongoRepo;
    private readonly UploadedDocumentRedisRepository _redisRepo;
    private readonly UploadedDocumentPostgresRepository _postgresRepo;
    private readonly ILogger<CompositeUploadedDocumentRepository> _logger;

    public CompositeUploadedDocumentRepository(
        UploadedDocumentMongoRepository mongoRepo,
        UploadedDocumentRedisRepository redisRepo,
        UploadedDocumentPostgresRepository postgresRepo,
        ILogger<CompositeUploadedDocumentRepository> logger)
    {
    ...
    public async Task AddAsync(UploadedDocument entity, CancellationToken cancellationToken = default)
    {
    ...
    public async Task UpdateAsync(UploadedDocument entity, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IReadOnlyList<UploadedDocument>> GetByApplicationIdAsync(AppId applicationId, CancellationToken cancellationToken = default)
    {
        try
        {
            var fromMongo = await _mongoRepo.GetByApplicationIdAsync(applicationId, cancellationToken);
            if (fromMongo.Any()) return fromMongo;
        }
        catch (Exception ex) { _logger.LogWarning(ex, "Mongo lookup failed, trying Postgres"); }
        
        return await _postgresRepo.GetByApplicationIdAsync(applicationId, cancellationToken);
    }
    ...
    public async Task<IReadOnlyList<UploadedDocument>> GetByDocumentTypeAsync(string documentType, CancellationToken cancellationToken = default)
    {
        try
        {
            var fromMongo = await _mongoRepo.GetByDocumentTypeAsync(documentType, cancellationToken);
            if (fromMongo.Any()) return fromMongo;
        }
        catch (Exception ex) { _logger.LogWarning(ex, "Mongo lookup failed, trying Postgres"); }
        
        return await _postgresRepo.GetByDocumentTypeAsync(documentType, cancellationToken);
    }
    ...
    public async Task<IReadOnlyList<UploadedDocument>> GetByApplicationIdAndTypeAsync(AppId applicationId, string documentType, CancellationToken cancellationToken = default)
    {
        try
        {
            var fromMongo = await _mongoRepo.GetByApplicationIdAndTypeAsync(applicationId, documentType, cancellationToken);
            if (fromMongo.Any()) return fromMongo;
        }
        catch (Exception ex) { _logger.LogWarning(ex, "Mongo lookup failed, trying Postgres"); }
        
        return await _postgresRepo.GetByApplicationIdAndTypeAsync(applicationId, documentType, cancellationToken);
    }
