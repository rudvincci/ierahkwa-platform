...
{
    private readonly IContentRepository _postgresRepository;
    private readonly ContentMongoRepository _mongoRepository;
    ...
    public ReadModelSyncService(
        IContentRepository postgresRepository,
        ContentMongoRepository mongoRepository,
    ...
    public async Task SyncContentAsync(Content content, CancellationToken cancellationToken = default)
    {
        var stopwatch = Stopwatch.StartNew();
        
        try
        {
            var exists = await _mongoRepository.ExistsAsync(content.Id, cancellationToken);
            
            if (exists)
            {
                await _mongoRepository.UpdateAsync(content, cancellationToken);
            }
            else
            {
                await _mongoRepository.AddAsync(content, cancellationToken);
            }
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Failed to sync Content {ContentId} to MongoDB", content.Id.Value);
            throw;
        }
    }

    public async Task RemoveContentAsync(ContentId contentId, CancellationToken cancellationToken = default)
    {
        var stopwatch = Stopwatch.StartNew();
        
        try
        {
            await _mongoRepository.DeleteAsync(contentId, cancellationToken);
            stopwatch.Stop();
            UpdateMetrics(stopwatch.Elapsed, success: true);
        }
        catch (Exception ex)
        {
            stopwatch.Stop();
            UpdateMetrics(stopwatch.Elapsed, success: false);
            _logger.LogError(ex, "Failed to remove Content {ContentId} from MongoDB", contentId.Value);
            throw;
        }
    }
