...
Portal default URL: `https://localhost:5181` (HTTPS) or `http://localhost:5180` (HTTP)
...
- **Citizen**: `https://localhost:5181/dev/mock-login?tenant=default&user=citizen@example.com&role=Citizen`
- **Government Agent**: `https://localhost:5181/dev/mock-login?tenant=default&user=agent@example.com&role=GovernmentAgent`
- **Admin**: `https://localhost:5181/dev/mock-login?tenant=default&user=admin@example.com&role=Admin`
- **Content Editor**: `https://localhost:5181/dev/mock-login?tenant=default&user=editor@example.com&role=ContentEditor`
- **Library Editor**: `https://localhost:5181/dev/mock-login?tenant=default&user=library@example.com&role=LibraryEditor`

After setting the cookie, hit:

- **Who am I** (dev): `https://localhost:5181/dev/whoami`
...
- `https://localhost:5181/auth/login` (Blazor login page)
  - will force-load to `https://localhost:5181/auth/oidc/login` in OIDC mode

Logout:

- `https://localhost:5181/auth/logout` (Blazor logout)
  - will hit `https://localhost:5181/auth/oidc/logout` in OIDC mode
...
- an admin-managed mapping in the portal: `https://localhost:5181/gov/user-tenant-mappings`
  - use **"Use my identity"** while logged in to prefill (Issuer + Subject)

If your resolved tenant doesn't exist:

- **Admin** users are redirected to: `https://localhost:5181/gov/tenants?prefillTenantId=...`
...
- `https://localhost:5181/dev/whoami`
